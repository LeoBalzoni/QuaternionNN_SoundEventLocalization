...
class FeatureClass:
    def __init__(self, dataset='ansim', ov=3, split=1, nfft=1024, db=30, wav_extra_name='', desc_extra_name=''):
        ...
        if dataset == 'ansim':
            self._base_folder = 'NN/base_folder/'
        elif dataset == 'resim':
        ...
        self._fs = 48000  # With ANSIM RESIM and REAL is 44100
        ...
        self._ele_list = range(-60, 60, self._resolution)  # With TAU is -40,40
        ...
        self._audio_max_len_samples = 60 * self._fs  # TODO: Fix the audio synthesis code to always generate 30s of      # TODO originale è 30 + fs
    ...
    # Qfft taken from https://github.com/jflamant/bispy/tree/master/bispy da riga 139 a 167
    ...
    def sympSynth(self, q_1, q_2):
        ...
        return qfloat
        # quaternion.as_quat_array(qfloat)
    ...
    def _read_desc_file(self, desc_filename):
        ...
        for line in fid:
            ...
            if 'tau' in self._dataset:
                desc_file['class'].append(split_line[0])  # TODO aggiunto questo
            else:
    ...
    def get_label_dir(self, mode, weakness, extra=''):
        return os.path.join(
            self._base_folder,
            'label_ov{}_split{}_nfft{}_{}{}{}'.format(self._ov, self._split, self._nfft, mode,
                                                      0 if mode is 'regr' else weakness, extra)
        )
